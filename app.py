...
def gemini_generate_text(
    ...
    """シンプルなテキスト生成ヘルパー（新 Gemini SDK 対応）"""
...
def analyze_kai(kai_list):
    """
    保存済みの快ログをもとに、シンプルなAI分析を返す（MVP安定版）。

    盛り込み内容:
    - 直近7日分を優先して「今の状態」を分析
    - 全体データも参考情報として渡す
    - 出力は400〜600文字程度に制御
    - Geminiのmax_tokensは1500にして途中切れを防ぐ
    - 医療・診断っぽい断定表現を避ける
    """
    if not kai_list:
    ...
    for log in kai_list:
        ...
        created_at = (
            created_at_dt.strftime("%Y-%m-%d %H:%M")
            if created_at_dt else "日時不明"
        )

        row = f"- {name}（{count}回 / 最終追加: {created_at}）"
        all_rows.append(row)
    ...
    # 直近7日がなければ、全体データを使って分析する
    recent_text = "\n".join(recent_rows) if recent_rows else "直近7日以内の快ログはまだありません。"
    all_text = "\n".join(all_rows)

    prompt = f"""
あなたは「マインドクエスト」という自己理解アプリのAI分析役です。
以下はユーザーが「良い・心地よい・自分に合う」と感じたことの記録です。
...
このデータから、MVP検証用としてシンプルに分析してください。
...
・ユーザーを決めつけない
・医療・診断・治療のような表現は避ける
・「〜かもしれません」「〜が合いやすそうです」のように柔らかく書く
・おすすめ行動は小さく、今日すぐできるものにする
・余計な前置きや締めの挨拶は不要

【出力形式】

🧠 あなたの快傾向
2〜3文で、どんなことで満たされやすそうかを書いてください。

⚠️ 最近不足しているかもしれない快
1〜2文で、最近少なそうな要素や、意識するとよさそうな要素を書いてください。
直近7日のログが少ない場合は「まだ判断材料は少ないですが」と添えてください。
...
def add_xp(user, amount: int, reason: str = ""):
    ...
    print(
        f"XP +{amount}: {reason} / "
        f"old_xp={old_xp}, total={user.xp}, "
        f"old_level={old_level}, level={user.level}"
    )
...
def ensure_extra_columns():
    ...
    try:
        ...
        db.session.commit()
        print("Extra columns checked: xp OK")
    except Exception as e:
        db.session.rollback()
        print("ensure_extra_columns error:", e)
...
def inject_global_status():
    ...
    try:
        ...
        print("inject_global_status error:", e)
        return {}
...
def kai_status():
    ...
    # 画面自体はテンプレ＆JSで描画、ここでは user と analysis 初期値だけ渡す
    return render_template("kai_status.html", user=user, analysis=None)


@app.route("/kai_analyze")
def kai_analyze():
    """快ログをAI分析して kai_status.html に表示する。"""
    user = ensure_user()
...
"""
    ...
    print("ai_comment:", repr(ai_comment))
    print("==================")
